...
#define MODBUS_RPACKET_MIN_LEN 		0x04

// REQUEST
#define MODBUS_RADDRESS_OFFSET 		0x00
#define MODBUS_RFUNCTION_OFFSET 	0x01
#define MODBUS_RDATA_OFFSET 		0x02

// ANSWER
#define MODBUS_AADDRESS_OFFSET 		0x00
#define MODBUS_AFUNCTION_OFFSET 	0x01
#define MODBUS_ABYTES_OFFSET		0x02
...
size_t modbus_start_answer(struct modbus_instance *instance, uint8_t function)
{
	...
	instance->send_buffer[MODBUS_AFUNCTION_OFFSET] = function;

	return 2;
}

size_t modbus_end_answer(struct modbus_instance *instance, size_t len)
{
	uint16_t crc_calculated = crc16(instance->send_buffer, len);
...
static int _modbus_find_regs_table(const struct modbus_regs_table *tables,
		uint16_t address, uint16_t count, uint16_t **regs)
{
	int i = 0;
	while (tables[i].address != 0
			|| tables[i].count != 0
		...
		enum modbus_table table)
{
	...
	if (_modbus_find_regs_table((table == MODBUS_TABLE_INPUT_REGISTERS? instance->input_tables: instance->holding_tables),
			command.address, command.count, &regs) < 0)
	{
		modbus_send_error(instance, req->function, MODBUS_ERROR_ILLEGAL_DATA_ADDRESS);
...
int modbus_write_register_cmd(struct modbus_instance *instance, struct modbus_request *req)
{
	...
	if (_modbus_find_regs_table(instance->holding_tables, command.address, 1, &regs) < 0)
	{
		modbus_send_error(instance, req->function, MODBUS_ERROR_ILLEGAL_DATA_ADDRESS);
...
int modbus_write_multiple_regs_cmd(struct modbus_instance *instance, struct modbus_request *req)
{
	...
	if (_modbus_find_regs_table(instance->holding_tables, command.address, command.count, &regs) < 0)
	{
		modbus_send_error(instance, req->function, MODBUS_ERROR_ILLEGAL_DATA_ADDRESS);
	...
	for (i = command.address, j = 0; i < command.address + command.count; i++, j += 2) {
		/* 6 and 7 = first value */
		regs[i] = (req->data[MODBUS_WRITE_MULTIPLE_DATA_OFFSET + j] << 8)
...
int modbus_report_slave_id_cmd(struct modbus_instance *instance, struct modbus_request *req)
{
	...
	if (instance->id)
	{
		size_t slen = strlen((char *)instance->id);
...
int modbus_write_read_regs_cmd(struct modbus_instance *instance, struct modbus_request *req)
{
	...
	if (_modbus_find_regs_table(instance->holding_tables, command.read_address, command.read_count, &read_regs) < 0
			|| _modbus_find_regs_table(instance->holding_tables, command.write_address, command.write_count, &write_regs) < 0)
	{
		modbus_send_error(instance, req->function, MODBUS_ERROR_ILLEGAL_DATA_ADDRESS);
	...
	for (i = command.read_address; i < command.read_address + command.read_count; i++) {
		instance->send_buffer[len++] = read_regs[i] >> 8;
